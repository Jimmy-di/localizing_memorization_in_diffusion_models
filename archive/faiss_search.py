...
def main():
    ...
    for index in range(file_count):
        for second_i in range(0,5):
            ...
            for i, result in enumerate(results):
                ...
                try:
                    ...
                    im.save('./'+ dname + '/{}_{}_{}'.format(str(index), str(second_i), str(i))  + endings)
        ...
        dict = {'image name': img_names, 'caption': faiss_descriptions, 'score': faiss_score} 
    
        df = pd.DataFrame(dict)
        df.to_csv('./'+dname+'/report.csv', index=False)
    print(df) 
